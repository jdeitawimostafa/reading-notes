# summarizing : 

## chapter 4 (links) : 
there are a common types of links we used : 
1. Links from one website to another . 
2. Links from one page to another on the same website
3. Links from one part of a web page to another part of the
same page . 
4. Links that open in a new browser window
5. Links that start up your email program and address a new
email to someone . 

* we can create links by using a element and users can click  on anything
between the opening a tag and the closing a tag , 

* and we can use a element for the types of links we used like (from website to another , from one page to another on the same website)  . 

![html](https://i.ytimg.com/vi/IK35oTxuQz8/maxresdefault.jpg)

* there are two types of url : 
1. absolute url . 
2. relative url . 

* we can use mailto after a element : to create a link that starts up the user's email . 

![email](https://www.wikihow.com/images/thumb/3/34/Create-an-Email-Link-in-HTML-Step-5.jpg/v4-460px-Create-an-Email-Link-in-HTML-Step-5.jpg.webp) 

* we can opening links in a new window using bu using : target . 

*** 

### chapter 15 (layout) : 

css has the following schemes that allow you to control the layout of a page :
1. normal flow . 
3. relative positioning . 
4. absolute positioning . 

![schemes](https://www.internetingishard.com/html-and-css/advanced-positioning/css-positioning-schemes-790d5b.png)

*** 

#### chapter (functions , methods , objects ) : 

* *what is a functios* ? 

Functions are one of the fundamental building blocks in JavaScript. A function in JavaScript is similar to a procedure—a set of statements that performs a task or calculates a value, but for a procedure to qualify as a function, it should take some input and return an output where there is some obvious relationship between the input and the output. To use a function, you must define it somewhere in the scope from which you wish to call it.

*To create a function we have to get it a name and then write the statment nedded to acheive its task , this is called a functios declaration,
after declaration we have to do ***function call*** , so all of the statment will execute* . 

 *we can get a single value out of a function like when we want to calculate the area , even we can get a multiple value out of a function like when we want to calculate an area and volume* . 

![functions](data:image/png;base64,)

*** 
##### article of pair programming : 

* what is pair programming : is an agile software development technique in which two programmers work together at one workstation . 

* the pair programmer involves two roles : 
1. the drive : the Driver is the programmer who is typing and the only one whose hands are on the keyboard. 
2. the navigator : The Navigator uses their words to guide the Driver . 

###### thanks for reading . 




